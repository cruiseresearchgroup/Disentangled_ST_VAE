    ...
    for model in exp_models:
        ...
        if gamma:
            ...
            if model_solver.log_flag:
                model_solver.logger.debug('{}: set gamma to {}'.format(model, gamma))

        eval_iter = cfg['model_general'].getint('eval_iter')
        eval_metrics = json.loads(cfg['model_general']['eval_metrics'])
        
        dataloader_iterator = iter(model_solver.data_loader)
        for local_iter in range(model_solver.max_iter):
            try:
            ...
            model_solver.train_iter(x)
            if local_iter % eval_iter == 0:
                ...
                for metric_name, loss_dict in total_loss_dict.items():
                    ...
                    temp_dict['iter'] = local_iter
                    csv_path = './' + cfg.get('environment', 'log_dir') + '{}_{}_iter.csv'.format(model, metric_name)
